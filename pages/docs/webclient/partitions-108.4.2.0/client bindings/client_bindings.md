...
'collection' node collection {
	'download as import template': instruction
...
'operation' node operation {
	'reset': instruction
```
Clears the parameters and applies the defaults again. In contrast to 'reset',
the result of the last execution is preserved.
```js
	'clear parameters': instruction
	...
	'executable': stategroup {
		'yes' {
			'execute': instruction
```
Executes the operation, but leaves the parameters as they are, so that the
user can review the parameters that were used. Use 'clear parameters' to
clear them afterwards.
```js
			'execute and keep parameters': instruction
		}
		'no' { }
	}
}
...
```js
	...
	'cancel': instruction
	'discard': instruction
	'open view': instruction ( view , text )
	'node': binding 'node' switchable
	...
	'location': stategroup {
		...
		'remote' {
			'marked for deletion': stategroup {
				'yes' { }
				'no' { }
			}
		}
...
'node' node {
	...
	'apply input from': instruction ( node 'node' )
	'node': binding 'node'
	'entity': binding 'entity' switchable
...
'query' node query {
	...
	'text filter': binding 'query text filter'
	'collection filter': binding 'query collection filter'
```
...
```
A column can be hidden initially. And should not be rendered by default.
```js
		'hidden': stategroup {
			'no' { }
			'yes' { }
		...
		'type': stategroup {
			...
			'collection' {
				'collection column': binding 'query collection column' switchable
			}
			'widget' { }
		}
	}
...
'query entry' query entry {
	'open entry': instruction ( view , text )
		...
		'type': stategroup {
			...
			'state group' {
				'is set': stategroup {
					'yes' {
						'style': binding 'style' switchable
						'state name': text // the name in the application model
						'state': text // the translated version for display
					}
					'no' { }
				}
			}
			...
			'collection' {
				'result': binding 'query collection' switchable
			}
			'widget' { has-widget }
		}
	}
...
```
Negates the filter. With 'not in' the entries that do *not* match the
operation are returned. E.g. 'contains' becomes 'does not contain'. The
'containment' operation is not affected; it has its own operator.
```js
			'set operator in': instruction
			'set operator not in': instruction
...
'query collection column' query collection column { sortable
	'has filter': stategroup {
		'yes' {
			'filter': binding 'query collection filter' switchable
		}
		'no' { }
	}
}
...
'query text filter' query text filter {
	'set text contains': instruction ( text )
	'set text equals': instruction ( text )
	'set text starts with': instruction ( text )
	'set text ends with': instruction ( text )
...
```
Negates the filter. With 'not in' the entries that do *not* match the
operation are returned. E.g. 'contains' becomes 'does not contain'.
```js
	'set operator in': instruction
	'set operator not in': instruction
	'filter': binding 'query filter' switchable // the generic query part
	'operator': stategroup {
		'in' { }
		'not in' { }
	}
...
'query collection filter' query collection filter {
	'add key': instruction ( text )
	'clear': instruction
	'filter': binding 'query filter' switchable // the generic query part
	'candidates query': binding 'query' switchable
	'keys': collection {
		'remove': instruction
	}
}
'rule' node rule {
	...
	'status': stategroup {
		...
		'is busy resolving' { }
		'resolved' {
			'open entity': instruction ( view , text )
			'node': binding 'node' switchable
		}
	}
}
	...
	'navigable': stategroup {
		'yes' {
			...
			'status': stategroup {
				...
				'is busy resolving' { }
				'resolved' {
					'open reference': instruction ( view , text )
					'node': binding 'node' switchable
				}
			}
		}
...
'view' view {
	...
	'set focus target': instruction ( text )
	'node': binding 'node' switchable
	'entity': binding 'entity' switchable
	'focus target': stategroup {
		'none' { }
		'set' {
			'key': text
		}
