...
The result of an expression is described in the expression target. Depending on
the context of the expression, one or more targets are valid. For example
`none` is not valid for a `number` target.

### none

Do not yield a value. For example when populating collections do not yield an
entry.

### reset

Reset re-initializes a widget property to it's original value.

Only mutable widget properties can be modified using instructions. Hence reset
can only be used inside an instruction expression.
...
'<span class="token string">expression target</span>' {
	'<span class="token string">type</span>': stategroup (
		...
		'<span class="token string">reset</span>' { [ <span class="token operator">reset</span> ] }
...
'<span class="token string">node binding</span>' { [ <span class="token operator">(</span>, <span class="token operator">)</span> ]
	'<span class="token string">attributes</span>': dictionary {
		'<span class="token string">materialization</span>': [ <span class="token operator">=</span> ] stategroup (
			'<span class="token string">deferred</span>' { [ <span class="token operator">deferred</span> ] }
			'<span class="token string">immediate</span>' { }
		)
...
'<span class="token string">context selection</span>' {
	'<span class="token string">change context to</span>': stategroup (
		...
		'<span class="token string">containing view</span>' { [ <span class="token operator">view</span> ]
			'<span class="token string">view binding</span>': reference
		}
		'<span class="token string">other context</span>' {
