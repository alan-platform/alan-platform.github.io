...
For that purpose, you need an annotation after the `=` sign, and before the derivation expression itself: `= ( recurse /* ^ path to graph ^*/ '<graph>' ) ...expression...`.

If you add the the annotation to your expression, the expression can only refer to sibling references that partake in the specified graph `'<graph>'`.
If your expression requires traversal of multiple graphs, you can just split it up into multiple parts, and combine results in a separate computation.

Note that a single expression can only traverse
