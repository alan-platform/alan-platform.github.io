...
###### Connection status
The application engine exposes three different connection statuses for external sources.
...
Node types contain a `permissions definition` for controlling read and update permissions,
 and `attributes`.
A state carries a `todo definition` on top of that, to mark the state as a `todo`-item.
	...
	'<span class="token string">attributes</span>': dictionary {
		...
		'<span class="token string">type</span>': stategroup (
			'<span class="token string">reference set</span>' { [ <span class="token operator">:</span> <span class="token operator">reference-set</span> <span class="token operator">-></span> ]
				...
				'<span class="token string">referenced type path</span>': component <a href="#grammar-rule--plural-object-path">'plural object path'</a>
				'<span class="token string">value source</span>': [ <span class="token operator">=</span> ] stategroup (
					...
					'<span class="token string">aggregate branch</span>' {
				...
				'<span class="token string">type</span>': stategroup (
					...
					'<span class="token string">collection</span>' { [ <span class="token operator">collection</span> ]
						...
						'<span class="token string">type</span>': stategroup (
							...
							'<span class="token string">derived</span>' { [ <span class="token operator">=</span> ] // TODO@Alan-meta:: optional block... if new line start/line length exceeded, then format as block.
								'<span class="token string">recursion</span>': component <a href="#grammar-rule--recursion-annotation">'recursion annotation'</a>
...
For references from `Products` to other other `Products`, you need a special `siblings` navigation step.
...
'<span class="token string">entry reference definition</span>' {
	...
	'<span class="token string">tail</span>': [ <span class="token operator">[]</span> ] component <a href="#grammar-rule--descendant-object-path">'descendant object path'</a>
}
</pre>
</div>
</div>
	...
	'assembly': acyclic-graph
{
	...
	'Parts': collection ['Product'] {
		...
		'Product': text -> ^ siblings in 'assembly' []
	...
	'timeline': ordered-graph .'Is First Order' ( ?'Yes'|| ?'No'>'Previous Order' )
{
	...
	'Is First Order': stategroup (
		'Yes' { }
		'No' {
			'Previous Order': text -> ^ siblings in 'timeline' []
		}
	)
		...
		'<span class="token string">aggregate</span>' {
			'<span class="token string">type</span>': stategroup (
				'<span class="token string">count</span>' { [ <span class="token operator">count</span> ]
					'<span class="token string">nodes path</span>': [ <span class="token operator">(</span>, <span class="token operator">)</span> ] component <a href="#grammar-rule--plural-object-path">'plural object path'</a>
				}
...
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">branch condition</span>' {
...
'<span class="token string">collection expression</span>' {
	'<span class="token string">type</span>': stategroup (
		'<span class="token string">partition</span>' { [ <span class="token operator">partition</span> ]
			'<span class="token string">granularity path</span>': component <a href="#grammar-rule--variablized-object-path">'variablized object path'</a>
			'<span class="token string">number key property</span>': [ <span class="token operator">as</span> ] reference
			'<span class="token string">has order</span>': stategroup (
				'<span class="token string">no</span>' { }
				'<span class="token string">yes</span>' { [ <span class="token operator">in</span> ]
					'<span class="token string">graph</span>': reference
				}
			...
			'<span class="token string">branches</span>': [ <span class="token operator">(</span>, <span class="token operator">)</span> ] dictionary {
				...
				/* A partition branch groups the '<span class="token string">path</span>' results (the branch reference-set members)
				** on the number that the '<span class="token string">grouping path</span>' selects:
				** '<span class="token string">per size</span>' = .'<span class="token string">c</span>'* as $ on $ .'<span class="token string">size</span>' */
				'<span class="token string">path</span>': [ <span class="token operator">=</span> ] component <a href="#grammar-rule--plural-object-path">'plural object path'</a>
				'<span class="token string">variable assignment</span>': component <a href="#grammar-rule--optional-named-object-assignment">'optional named object assignment'</a>
				'<span class="token string">grouping path</span>': [ <span class="token operator">on</span> ] component <a href="#grammar-rule--variablized-object-path">'variablized object path'</a>
				'<span class="token string">repetition</span>': stategroup (
					'<span class="token string">no</span>' { }
					'<span class="token string">yes</span>' {
						'<span class="token string">interval path</span>': [ <span class="token operator">with</span> ] component <a href="#grammar-rule--variablized-object-path">'variablized object path'</a>
						/* The end is inclusive.
						** For example: a start value 1 `with` interval 3 `to` end value 10 will produce 1,4,7,10 */
						'<span class="token string">end path</span>': [ <span class="token operator">to</span> ] component <a href="#grammar-rule--variablized-object-path">'variablized object path'</a>
					}
				)
				'<span class="token string">condition</span>': component <a href="#grammar-rule--branch-condition">'branch condition'</a>
			}
		}
		'<span class="token string">flatten</span>' { [ <span class="token operator">flatten</span> ]
			'<span class="token string">branches</span>': [ <span class="token operator">(</span>, <span class="token operator">)</span> ] dictionary {
		...
		'<span class="token string">union</span>' { [ <span class="token operator">union</span> ]
			'<span class="token string">branches</span>': [ <span class="token operator">(</span>, <span class="token operator">)</span> ] dictionary {
				...
				/* A union branch either
				** - merges the '<span class="token string">path</span>' results into the union (entries correspond to the path results), or
				** - groups the '<span class="token string">path</span>' results (as branch reference-set members) on the node
				**   that the '<span class="token string">grouping path</span>' selects (entries correspond to distinct grouping nodes):
				** '<span class="token string">products from my orders</span>' = .'<span class="token string">My Orders</span>'* as $ on $ >'<span class="token string">Order</span>'>'<span class="token string">Product</span>' */
				'<span class="token string">path</span>': [ <span class="token operator">=</span> ] component <a href="#grammar-rule--plural-object-path">'plural object path'</a>
				'<span class="token string">type</span>': stategroup (
...
Note that a child initializer does not override a parent initializer.
...
In addition, you can express that a state creates a todo-item in a [todo-list](#todo-items) of your application.
...
root {
		...
		can-delete: user .'Type'?'Admin'
	{
		...
		'Type': stategroup (
			...
			'Employee' { }
			'Unknown'
				has-todo: user .'Type'?'Admin'
			{ }
		)
	}
	// only team members can read team information:
	'Teams': collection ['Name'] {
		can-read: .'Members'[ user .'ID']
...
But, because of `can-update: user is ( /*this*/ )` at the `Address` node type in the model, users can (only!) update their *own* `Address` information.

NOTE: for updating a property value, users require permissions to *read* all ancestor nodes!
...
The list of todo items is constructed from the states that you mark as todos.
You mark a state as a todo in your model with `has-todo:`, written after the state name and
before the state's node body, followed by an expression that specifies the users and/or
cases for which the todo item applies.
The requirement expression starts at the node of the state.
...
'Orders': collection ['ID'] {
	'ID': text
	'Product': text -> ^ .'Products'[]
	'Creation Time': number 'date and time' initializer: now
	'Status': stategroup (
		'New' {
...
{: #grammar-rule--object-initialization-behaviour }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">object initialization behaviour</span>' {
	'<span class="token string">behaviour</span>': stategroup (
	...
	'<span class="token string">operation</span>': stategroup (
		...
		'<span class="token string">produce value</span>' {
			'<span class="token string">value</span>': stategroup (
				...
				'<span class="token string">state</span>' {
					'<span class="token string">behaviour</span>': component <a href="#grammar-rule--object-initialization-behaviour">'object initialization behaviour'</a>
					'<span class="token string">state</span>': reference
					'<span class="token string">expression</span>': component <a href="#grammar-rule--command-object-expression">'command object expression'</a>
				}
				...
				'<span class="token string">empty collection</span>' { [ <span class="token operator">none</span> ] }
...
{: #grammar-rule--internal-initializer }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">internal initializer</span>' {
	'<span class="token string">initializer</span>': component <a href="#grammar-rule--command-object-expression">'command object expression'</a>
}
</pre>
</div>
			...
			'<span class="token string">type</span>': stategroup (
				...
				'<span class="token string">unary expression</span>' {
					'<span class="token string">type</span>': stategroup (
						...
						'<span class="token string">numerical type conversion</span>' {
							'<span class="token string">conversion</span>': [ <span class="token operator">from</span> ] reference
							'<span class="token string">expression</span>': [ <span class="token operator">(</span>, <span class="token operator">)</span> ] component <a href="#grammar-rule--scalar-expression">'scalar expression'</a>
						}
						'<span class="token string">sign inversion</span>' { [ <span class="token operator">-</span> ]
							'<span class="token string">expression</span>': component <a href="#grammar-rule--scalar-expression">'scalar expression'</a>
						}
				...
				'<span class="token string">binary expression</span>' {
					...
					'<span class="token string">expressions</span>': [ <span class="token operator">(</span>, <span class="token operator">)</span> ] group {
						'<span class="token string">left</span>': [, <span class="token operator">,</span> ] component <a href="#grammar-rule--scalar-expression">'scalar expression'</a>
						'<span class="token string">right</span>': component <a href="#grammar-rule--scalar-expression">'scalar expression'</a>
					}
				}
			)
		}
...
'<span class="token string">descendant base property path</span>' {
	'<span class="token string">path</span>': component <a href="#grammar-rule--descendant-property-path">'descendant property path'</a>
}
</pre>
</div>
</div>
	...
	'<span class="token string">has steps</span>': stategroup (
		...
		'<span class="token string">yes</span>' {
			'<span class="token string">type</span>': stategroup (
				...
				'<span class="token string">siblings set</span>' { [ <span class="token operator">siblings</span> ]
...
'<span class="token string">plural object path tail</span>' {
	'<span class="token string">has steps</span>': stategroup (
		'<span class="token string">no</span>' { }
		'<span class="token string">yes</span>' {
			'<span class="token string">entries step</span>': component <a href="#grammar-rule--collection-entries-step">'collection entries step'</a>
...
{: #grammar-rule--ui-icon }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">ui icon</span>' {
...
{: #grammar-rule--ui-description }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">ui description</span>' {
...
{: #grammar-rule--ui-optional-style }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">ui optional style</span>' {
...
{: #grammar-rule--ui-optional-label }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">ui optional label</span>' {
	'<span class="token string">has label</span>': stategroup (
		'<span class="token string">no</span>' { }
		'<span class="token string">yes</span>' { [ <span class="token operator">@label:</span> ]
			'<span class="token string">label expression</span>': component <a href="#grammar-rule--ui-scalar-value-expression">'ui scalar value expression'</a>
		}
	)
}
</pre>
...
'<span class="token string">ui action attribute</span>' {
	'<span class="token string">description</span>': component <a href="#grammar-rule--ui-description">'ui description'</a>
	'<span class="token string">icon</span>': component <a href="#grammar-rule--ui-icon">'ui icon'</a>
}
</pre>
</div>
</div>
...
'<span class="token string">ui scalar value expression</span>' {
	'<span class="token string">type</span>': stategroup (
		...
		'<span class="token string">number</span>' {
			'<span class="token string">type</span>': stategroup (
				...
				'<span class="token string">unary expression</span>' {
					'<span class="token string">type</span>': stategroup (
						...
						'<span class="token string">numerical type conversion</span>' { [ <span class="token operator">from</span> ]
							...
							'<span class="token string">expression</span>': [ <span class="token operator">(</span>, <span class="token operator">)</span> ] component <a href="#grammar-rule--ui-scalar-value-expression">'ui scalar value expression'</a>
						}
						'<span class="token string">sign inversion</span>' { [ <span class="token operator">-</span> ]
							'<span class="token string">expression</span>': component <a href="#grammar-rule--ui-scalar-value-expression">'ui scalar value expression'</a>
						}
				...
				'<span class="token string">binary expression</span>' {
					...
					'<span class="token string">expressions</span>': [ <span class="token operator">(</span>, <span class="token operator">)</span> ] group {
						'<span class="token string">left</span>': [, <span class="token operator">,</span> ] component <a href="#grammar-rule--ui-scalar-value-expression">'ui scalar value expression'</a>
						'<span class="token string">right</span>': component <a href="#grammar-rule--ui-scalar-value-expression">'ui scalar value expression'</a>
					}
				}
			)
		}
		...
		'<span class="token string">keys</span>' {
			'<span class="token string">source</span>': stategroup (
				'<span class="token string">key reference candidates</span>' { [ <span class="token operator">candidates</span> ]
					'<span class="token string">source</span>': stategroup (
						'<span class="token string">key reference</span>' { }
...
{: #grammar-rule--ui-scalar-default-list }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">ui scalar default list</span>' {
	...
	'<span class="token string">has fallback</span>': stategroup (
		'<span class="token string">yes</span>' { [ <span class="token operator">||</span> ]
			'<span class="token string">fallback</span>': component <a href="#grammar-rule--ui-scalar-default-list">'ui scalar default list'</a>
		}
		'<span class="token string">no</span>' { }
	)
}
...
Defaults come in two different flavors: `@default:` and `@expected:`.
In the GUI, they behave similarly: both are for providing the initial value for a property when it is constructed.
That is, assuming that expression evaluation leads to a valid result.

However, if an `@expected:` value can be determined, the GUI uses **read** mode for the property value.
In **read** mode, the user must manually enable **edit** mode to modify the value.
When a user overwrites the expected or default value, both default types behave identical: the user value is used and never overwritten.

Note that an `@expected:` value is by no means enforced upon the user.
In the future, we consider showing to the user whether an actual value is in sync with the expected value (in edit mode), or not.
This is particularly useful when copying values from an external system (via interface import), which must only be updated on demand.

The presentation of default values in the GUI for new nodes depends on the combination
of `@default:`, `@expected:`, and any applicable `where` rules.
The property will be in **edit mode** by default, but switches to **read mode**
(requiring the user to manually enable editing) under specific conditions.
The behaviour is as follows:
- **Read mode** is used if `@expected:` is specified (which may be of the form `@expected: ignore`), and one of the following holds:
  - The `@expected:` expression produces a valid value.
  - **Exactly one value** satisfies all `where` rules specified at the property.
- **Edit mode** is used in all other cases, including:
  - When only `@default:` is specified.
  - When multiple values are possible.
  - When only `where` rules are specified, even when only one value satisfies all rules.

{: #grammar-rule--ui-default-type }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">ui default type</span>' {
	'<span class="token string">type</span>': stategroup (
		'<span class="token string">default</span>' { [ <span class="token operator">@default:</span> ] }
		'<span class="token string">expected</span>' { [ <span class="token operator">@expected:</span> ] }
	)
}
</pre>
</div>
...
'<span class="token string">ui scalar default</span>' {
	'<span class="token string">default</span>': stategroup (
		'<span class="token string">no</span>' { }
		'<span class="token string">yes</span>' {
			'<span class="token string">type</span>': component <a href="#grammar-rule--ui-default-type">'ui default type'</a>
			'<span class="token string">expression</span>': component <a href="#grammar-rule--ui-scalar-default-list">'ui scalar default list'</a>
		}
	)
}
</pre>
...
'<span class="token string">ui property classification</span>' {
	'<span class="token string">classification</span>': stategroup (
		...
		'<span class="token string">suppressed</span>' { [ <span class="token operator">@suppressed</span> ] }
...
{: #grammar-rule--ui-visibility }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">ui visibility</span>' {
...
{: #grammar-rule--ui-break-out }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">ui break out</span>' {
...
{: #grammar-rule--ui-emphasis }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">ui emphasis</span>' {
...
{: #grammar-rule--ui-reference-set-attribute }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">ui reference set attribute</span>' { dynamic-order
	'<span class="token string">visibility</span>': component <a href="#grammar-rule--ui-visibility">'ui visibility'</a>
	'<span class="token string">break out</span>': component <a href="#grammar-rule--ui-break-out">'ui break out'</a>
	'<span class="token string">description</span>': component <a href="#grammar-rule--ui-description">'ui description'</a>
}
</pre>
</div>
...
'<span class="token string">ui group property</span>' { dynamic-order
	...
	'<span class="token string">description</span>': component <a href="#grammar-rule--ui-description">'ui description'</a>
	'<span class="token string">icon</span>': component <a href="#grammar-rule--ui-icon">'ui icon'</a>
	'<span class="token string">style</span>': component <a href="#grammar-rule--ui-optional-style">'ui optional style'</a>
}
</pre>
</div>
</div>
### Collections
The `@default:` is followed by an expression producing keys that are to be added to the `collection` when it is created.
The keyword `candidates` expresses that the path described by the key reference is the path to the set of keys.
The set of `candidates` may also be based entirely on a `where` rule using `candidates &''`.
With additional `where` instructions, the candidate keys can be filtered using `where` rules:
In addition to using `candidates` indicated by a path at the key property,
it is also possible to initialize a collection with a static set of keys,
...
```js
	...
	'My Labels': collection ['Name'] {
		'Name': text -> ^ .'Labels'[]
	}
	'Issues': collection ['Issue Name'] {
		'Issue Name': text
		// default keys are candidates based on ^ ^ .'Labels' and filtered with rule 'Default'
		'Labels': collection ['Name'] @default: candidates where 'Default' {
			'Name': text -> ^ ^ .'Labels'[] as $
				where 'Default' ~> $ .'Default'?'Yes'
		}
		// default keys are candidates based on 'Mine' (^ ^ .'My Labels')
		'Labels: using My Labels': collection ['Name'] @default: candidates &'Mine' {
			'Name': text -> ^ ^ .'Labels'[]
				where 'Mine' ~> ^ ^ .'My Labels'[]
		}
		// default keys are a constant set of keys
		'Labels: constant': collection ['Name'] @default: ['A' 'B' 'C'] {
			'Name': text
		}
		// default is a single key based on a text property value
		'Labels: text': collection ['Name'] @default: .'Issue Name' {
			'Name': text
		}
	}
```
...
Currently, the annotation does nothing for collections on the root node.
...
The `@show-aggregate:` annotation is for showing aggregate values
at the bottom of the table for to the parent collection.
...
'<span class="token string">ui number property</span>' { dynamic-order
	'<span class="token string">classification</span>': component <a href="#grammar-rule--ui-property-classification">'ui property classification'</a>
	'<span class="token string">description</span>': component <a href="#grammar-rule--ui-description">'ui description'</a>
	'<span class="token string">emphasis</span>': component <a href="#grammar-rule--ui-emphasis">'ui emphasis'</a>
	'<span class="token string">icon</span>': component <a href="#grammar-rule--ui-icon">'ui icon'</a>
	'<span class="token string">style</span>': component <a href="#grammar-rule--ui-optional-style">'ui optional style'</a>
	'<span class="token string">default</span>': component <a href="#grammar-rule--ui-scalar-default">'ui scalar default'</a>
	...
	'<span class="token string">show aggregate</span>': stategroup (
		'<span class="token string">no</span>' { }
...
'<span class="token string">ui file property</span>' { dynamic-order
	...
	'<span class="token string">file name expression</span>': stategroup (
		...
		'<span class="token string">yes</span>' { [ <span class="token operator">@name:</span> ]
			'<span class="token string">file name expression</span>': component <a href="#grammar-rule--ui-scalar-default-list">'ui scalar default list'</a>
		}
	)
}
</pre>
...
'<span class="token string">ui text property</span>' { dynamic-order
	...
	'<span class="token string">has validation</span>': stategroup (
		...
		'<span class="token string">yes</span>' { [ <span class="token operator">@validate:</span> ]
			// @validate: ^ ^ .'<span class="token string">Email regex</span>' ( ^ ^ .'<span class="token string">Email regex description</span>' )
			// @validate: switch .'<span class="token string">A</span>' (
			// 	|'<span class="token string">x</span>' => ""
			// 	|'<span class="token string">y</span>' => ""
			// ) ( "In geval van x dan y. Anders bla." )
			'<span class="token string">regular expression</span>': component <a href="#grammar-rule--ui-expression">'ui expression'</a>
			'<span class="token string">has description</span>': stategroup (
				'<span class="token string">no</span>' { }
...
### Todo items
A todo item carries three annotations, in any order.
`@label:` is the text that names the item in the list of todo items.
`@description:` is the text shown with the item.
`@style:` colors the item.
The webclient shows todo items in a section of their own, away from the states that carry them,
so the expressions of `@label:` and `@style:` both start at the root of your dataset.

{: #grammar-rule--ui-todo }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">ui todo</span>' { dynamic-order
	'<span class="token string">label</span>': component <a href="#grammar-rule--ui-optional-label">'ui optional label'</a>
	'<span class="token string">description</span>': component <a href="#grammar-rule--ui-description">'ui description'</a>
	'<span class="token string">style</span>': component <a href="#grammar-rule--ui-optional-style">'ui optional style'</a>
}
</pre>
...
### Numerical types
The custom numerical type can be displayed in four different styles:
- in the default decimal notation,
- as a currency: to indicate the currency, the ISO code (https://en.wikipedia.org/wiki/ISO_4217) is expected to be found in the 'label' property. The formatting is done according to the browsers locale settings.
- in scientific notation, or
...
'<span class="token string">ui numerical type</span>' {
	'<span class="token string">represent as</span>': stategroup (
		...
		'<span class="token string">custom type</span>' {
			'<span class="token string">style</span>': stategroup (
				'<span class="token string">decimal</span>' { [ <span class="token operator">@numerical-type:</span> ] }
