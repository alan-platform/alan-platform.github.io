...
global-features
	...
	'layout' {
		...
		'advanced features': stategroup @default: 'no' (
			'no' { }
			'yes' { }
		)
		'show rules': stategroup @default: 'no' (
			'no' { }
			'yes' { }
		)
		'show hidden properties': stategroup @default: 'no' (
			'no' { }
			'yes' { }
		)
	}
...
node-features
	...
	'2d chart query' component {
		...
		'lines': list {
			...
			'cumulative': stategroup @default: 'no' (
				'no' { }
				'yes' { }
			)
	...
	/// The properties of a query are its columns: they are shown in the table and can be
	/// sorted and filtered. Every property has a label, and 'number', 'text' and
	/// 'stategroup' properties also take a filter.
	'properties' component list stategroup @strategy (
		'stategroup' binding stategroup {
			'label': stategroup @default: 'auto' @strategy (
				'auto' { }
				'set' text
			)
			/// Filter on the state of this stategroup.
			'filter': stategroup @default: 'auto' (
				/// Let the generator decide: a filter on a query or a custom query, and no
				/// filter on the query of a 'node' feature, which is a detail view of a
				/// single node.
				'auto' { }
				/// Only show entries that are in one of the 'states to include'. The user
				/// cannot change this filter.
				'yes' {
					'states to include': list binding state { }
				}
				/// The user selects the states to include.
				'simple' { }
		...
		'number' binding number {
			...
			'filter': feature 'optional number filter'
		}
		'text' binding text {
			'label': feature 'query property key'
			'filter': feature 'optional text filter'
		}
		'file' binding file {
			'label': feature 'query property key'
		}
	...
	'text filter' component stategroup @default: 'auto' (
		...
		'yes' {
			'criteria': text
			/// Use 'not in' to exclude the rows that match the criteria.
			'operator': stategroup @default: 'in' (
				'in' { }
				'not in' { }
			)
		}
		...
		'simple' {
			/// Use 'not in' to exclude the rows that match what the user fills in.
			'operator': stategroup @default: 'in' (
				'in' { }
				'not in' { }
			)
	...
	// Optional variants of the filters above, adding the option of no filter at all. Only
	// 'properties' uses these, so they are not part of the common features.
	/// Filter on this number property, or use 'none' for a column without a filter.
	'optional number filter' component stategroup @default: 'auto' @strategy (
		/// Let the generator decide: a simple filter on a query or a custom query, and no
		/// filter on the query of a 'node' feature, which is a detail view of a single
		/// node. This is the default, so a property without a filter needs no
		/// configuration.
		'auto' { }
		'set' feature 'number filter'
		/// Do not filter on this property.
		'none' none
	)

	/// Filter on this text property, or use 'none' for a column without a filter.
	'optional text filter' component stategroup @default: 'auto' @strategy (
		/// Let the generator decide: the filter of the 'auto' state of 'text filter' on a
		/// query or a custom query, and no filter on the query of a 'node' feature, which
		/// is a detail view of a single node. This is the default, so a property without a
		/// filter needs no configuration.
		'auto' { }
		'set' feature 'text filter'
		/// Do not filter on this property.
		'none' none
	)
	...
	'filters' component list non-empty stategroup @strategy (
		'stategroup' binding stategroup {
			// A component feature cannot be referenced from a stategroup context, so the label
			// and the filter are inlined here.
			'label': stategroup @default: 'auto' @strategy (
				'auto' { }
				'set' text
			)
	...
	'flowchart' {
		'orientation': stategroup (
			'horizontal' none
			'vertical' none
		)
		'collection': binding nodes {
			'label': binding text { } //todo: make label
			'background color': feature 'planning color'
			'predecessors': binding nodes {
				'reference': binding text reference { }
			}
		}
	}
	...
	'custom query' component {
		'name': text
		'select by default': stategroup @default: 'no' (
			'no' { }
			'yes' { }
		)
		'include dormant filter': stategroup @default: 'no' (
			'no' { }
			'yes' { }
		)
		'custom sorting': feature 'optional sorting'
...
collection-features
	'comments' {
		'author': binding text { }
		'timestamp': binding number { }
		'body': binding text { }
		'attachments': stategroup @default: 'not set' @strategy (
			'not set' none
			'set' binding collection {
				'file name': binding text { }
				'file': binding file { }
			}
		)
	}

	/// Custom filters are added as extra toggle filters on the collection. They are
	/// only added on the 'compact' and 'full' views. For custom query filters are
	/// specified separately.
	'custom filters' stategroup @default: 'not set' @strategy (
		'set' feature 'filters'
		'not set' none
	)

	'custom queries' list feature 'custom query'
...
'<span class="token string">extended annotations</span>' { [ <span class="token operator">(</span>, <span class="token operator">)</span> ]
	...
	'<span class="token string">has handheld frames</span>': stategroup (
		'<span class="token string">yes</span>' {
			'<span class="token string">handheld frames</span>': dictionary { [ <span class="token operator">view:</span> ]
		...
		'<span class="token string">view</span>' {
			...
			'<span class="token string">type</span>': stategroup (
				'<span class="token string">frame</span>' {
					'<span class="token string">frame</span>': [ <span class="token operator">view:</span> ] reference
				}
				'<span class="token string">command</span>' {
					'<span class="token string">command</span>': [ <span class="token operator">(</span>, <span class="token operator">)</span> ] component <a href="#grammar-rule--handheld-view-command-selector">'handheld view command selector'</a>
				}
			)
		}
		'<span class="token string">none</span>' { }
...
'<span class="token string">handheld view frame</span>' {
	'<span class="token string">type</span>': stategroup (
		...
		'<span class="token string">feature</span>' {
			'<span class="token string">definition</span>': [ <span class="token operator">feature</span> ] reference
			'<span class="token string">feature</span>': component <a href="#grammar-rule--feature">'feature'</a>
		}
	)
}
...
'<span class="token string">handheld view selector</span>' {
	'<span class="token string">type</span>': stategroup (
		...
		'<span class="token string">command</span>' {
			'<span class="token string">annotation</span>': component <a href="#grammar-rule--handheld-view-command-selector">'handheld view command selector'</a>
		}
		'<span class="token string">view</span>' {
			'<span class="token string">emphasis</span>': [ <span class="token operator">:</span> ] stategroup (
				'<span class="token string">low</span>' { }
...
{: #grammar-rule--handheld-view-command-selector }
<div class="language-js highlighter-rouge">
<div class="highlight">
<pre class="highlight language-js code-custom">
'<span class="token string">handheld view command selector</span>' {
