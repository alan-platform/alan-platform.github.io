...
'<span class="token string">archetype</span>': stategroup (
	'<span class="token string">scheduled provider</span>' { [ <span class="token operator">provider</span> ]
		 ...
		 * Optionally, command handlers can be provided. Currently only commands defined on the interface root node are supported.
		 */
		/* Main routine.
		 ...
		 * The command arguments are available in `$`.
		 */
		'<span class="token string">command routines</span>': dictionary { [ <span class="token operator">command</span> ]
			...
			'<span class="token string">type</span>': stategroup (
				...
				'<span class="token string">schedule</span>' { [ <span class="token operator">schedule</span> ]
					 ...
					 * This run of the main routine is in addition to runs causes by the external schedule.
					 */
				}
			)
		}
	...
	'<span class="token string">consumer</span>' { [ <span class="token operator">consumer</span> ]
		...
		'<span class="token string">routines</span>': dictionary { [ <span class="token operator">routine</span> ]
			...
			/* The path to the node the routine binds to or for event is defined at.
...
'<span class="token string">schema scalar type</span>' {
	'<span class="token string">type</span>': stategroup (
		...
		'<span class="token string">text</span>' { [ <span class="token operator">text</span> ]
			 ...
			 * No assumptions are made about the content of text data or its encoding.
			 * Two texts are only considered equal when their binary data is identical.
			 * Two texts containing the same text, but in different and incompatible encodings, are not considered equal.
			 */
		}
...
'<span class="token string">schema complex type</span>' {
	'<span class="token string">type</span>': stategroup (
		...
		'<span class="token string">node</span>' {
			/* Node type.
			 * Parsing and serialization expected the type information (property names) to be present in the data.
			 */
...
'<span class="token string">stack block</span>' {
	 ...
	 * With a `let` expressions it is possible to store a value in a name.
		...
		'<span class="token string">non-empty</span>' {
			'<span class="token string">select</span>': stategroup (
				'<span class="token string">this frame</span>' {
					/* Select a value on the selected frame.
					 */
...
'<span class="token string">pattern rule</span>' { [ <span class="token operator">(</span>, <span class="token operator">)</span> ]
	 ...
	 * A pattern is a set of parts, each part must capture a piece.
...
'<span class="token string">promise</span>' {
	...
	'<span class="token string">head</span>': stategroup (
		...
		'<span class="token string">list merge</span>' {
			/* This operations are identical to the equivalently named operations in promise-chain's `reduce` instruction.
				...
				'<span class="token string">state</span>' {
					'<span class="token string">type</span>': stategroup (
						'<span class="token string">interface</span>' { [ <span class="token operator">create</span> ]
							/* Set an Alan Interface `stategroup` to the specified state.
							 */
...
'<span class="token string">statement</span>' {
	'<span class="token string">type</span>': stategroup (
		...
		'<span class="token string">execute command</span>' {
			/* Execute a Alan Interface `command`.
			 * Only consumer archetype processors can execute commands.
			 * The command is send to the Alan Interface provider.
			 */
