...
In this small data model (known as command parameters) inside our main model, we can express references to the main model, like `-> ^ ^ .'Management'.'Tables'[]`.
...
After the parameter node and naming the node with a named object, we express the operations that are to be performed, like `... => update .'Orders' ...`.
...
The command implementation concludes with `... = create ( ... )`.
