...
if [ -n "${ALAN_DEVENV:-}" ]; then
  ...
  [ -f "$ALAN_DEVENV/system-types/datastore/migration/language" ] || echo "warning: datastore migration language missing" >&2
  printf '%s\n' "$ALAN_DEVENV"; exit 0
fi
CACHE="$ROOT/.toolchains/$V"
DEVENV="$CACHE/.alan/devenv"
if [ -x "$DEVENV/platform/project-compiler/tools/compiler-project" ] && [ -f "$DEVENV/system-types/datastore/migration/language" ]; then printf '%s\n' "$DEVENV"; exit 0; fi
...
jq -e '."system types".datastore.version' versions.json >/dev/null || { echo "error: dist versions.json for $V lists no datastore system type" >&2; exit 1; }
...
[ -f "$DEVENV/system-types/datastore/migration/language" ] || { echo "error: datastore migration language missing after fetch" >&2; exit 1; }
printf '%s\n' "$DEVENV"
